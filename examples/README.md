> Note: all examples are run using images and palette from `example_cartridge`

A lil pieces of code to serve as examples for different instructions and concepts:
- [copy.vea](copy.vea) - shows how to use mov and copy instructions to efficiently move data around
- [additive.vea](additive.vea) - shows loops and branching and how to use jump, arithmetic and binary instructions
- [input.vea](input.vea) - shows how to manipulate input byte to create a little game of square moving around
- [func.vea](func.vea) - shows how to create and call functions
- [example_cartridge](example_cartridge) - shows cartridge format
- [images.vea](images.vea) - shows how to render backgrounds and sprites